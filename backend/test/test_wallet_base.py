...
class MockWallet(Wallet):
    ...
    def __init__(self, name: str, id: str, reference_fiat: str, portfolio_id: str = "test_portfolio", description: str = "", api_key: str = None, api_secret: str = None):
...
class TestWalletBaseMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.wallet = MockWallet("My Mock Wallet Name", "MOCK-1", "EUR", "test_portfolio", "My Mock Wallet Description")
    ...
    def test_retrieve_local_ledger_data_file_exists_with_valid_data(self):
        ...
        test_data = {
            'datetime': [datetime(2022, 1, 1, 12, 0, 0), datetime(2022, 1, 2, 12, 0, 0)],
        ...
        test_df.to_parquet(self.wallet.ledger_file, engine="fastparquet", compression="GZIP")
        ...
        self.assertEqual(result['asset_price_in_reference_fiat'].iloc[0], Decimal('50000.0'))
    ...
    def test_retrieve_local_ledger_data_with_numeric_strings(self):
        ...
        self.assertEqual(result['asset_price_in_reference_fiat'].iloc[0], Decimal('50000.50'))
        self.assertEqual(result['fee'].iloc[0], Decimal('0.001'))
    ...
    def test_retrieve_local_ledger_data_column_filtering(self):
        ...
        for col in extra_columns:
            self.assertNotIn(col, result.columns, f"Extra column '{col}' should not be present")
    ...
    def test_save_local_ledger_data_column_filtering(self):
        ...
        for col in required_columns:
            self.assertIn(col, saved_df.columns, f"Required column '{col}' is missing in saved file")
        
        # Verify extra columns are not present in saved file
        extra_columns = ['extra_column_1', 'extra_column_2', 'unused_column']
        for col in extra_columns:
            self.assertNotIn(col, saved_df.columns, f"Extra column '{col}' should not be present in saved file")
    ...
    def test_get_balance_with_valid_data(self, mock_get_ohlc_data):
        ...
        timestamp_now_for_mock = datetime.now().timestamp() - 2 * RESAMPLING_INTERVAL_IN_SECONDS
        mock_get_ohlc_data.return_value = self._simulate_ohlc_data(timestamp_now_for_mock, 3)
        ...
        required_columns = [
            'asset', 'balance', 'balance_in_reference_fiat', 
            'asset_price_in_reference_fiat', 'timestamp'
        ]
        for col in required_columns:
            self.assertIn(col, result.columns, f"Required column '{col}' is missing")
