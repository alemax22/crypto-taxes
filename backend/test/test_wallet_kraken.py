...
from wallets.wallet_kraken import EXCEPTION_ASSETS, KrakenWallet
...
class TestKrakenWallet(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.wallet = KrakenWallet(
            "My Kraken Wallet Name",
            "KRAKEN-1",
            "EUR",
            "test_portfolio",
            "My Kraken Wallet Description",
            self.test_api_key,
        ...
        self.wallet.ledger_file = os.path.join(
            self.persistent_data_dir, "data", "kraken_ledger.parquet"
        )

        # Also override OHLC file path for testing (keep filename, change base folder)
        self.wallet.ohlc_file = os.path.join(
            self.persistent_data_dir, "data", "kraken_ohlc.parquet"
        )
    ...
    def test_authenticate_failure_api_error(self, mock_get_ledger):
        ...
        mock_get_ledger.return_value = {"error": ["API:Invalid key"]}
    ...
    def test_synchronize_full_ledger_success(
        ...
        def mock_get_ledger_side_effect(start_timestamp, ofs, without_count="false"):
            ...
            with open(
                os.path.join(os.path.dirname(__file__), json_files[file_index]), "r"
            ) as f:
            ...
            if without_count == "false":
                ...
                return {"error": [], "result": {"ledger": ledger_data}}
        ...
        self.assertFalse(
            filter_mask.any(),
            "There are transactions without a price: "
            + str(saved_df[filter_mask].shape),
        )

        # Check that the balance column has always positive values
        self.assertTrue(
            (saved_df["balance"] >= 0).all(),
            "Balance column should have positive values",
        )
    ...
    def _simulate_tradable_assets_info(self):
        ...
        with open(
            os.path.join(
                os.path.dirname(__file__), "data", "_get_tradable_assets_info.json"
            ),
            "r",
        ) as file:
    ...
    def _verify_asset_mapping(self, df):
        ...
        self.assertIn(
            "asset_original_name", df.columns, "asset_original_name column should exist"
        )
    ...
    def _verify_transaction_type_mapping(self, df):
        ...
        for tx_type in expected_types:
            self.assertIn(
                tx_type,
                df["transaction_original_type"].values,
                f"Transaction type {tx_type} should be present in the data",
            )
